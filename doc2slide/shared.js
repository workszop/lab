...
/* display:block above out-specifies the UA's [hidden] rule, so the custom-id
   field has to opt back out explicitly or it never hides. */
.ai-dialog .ai-field[hidden]{display:none;}
...
.ai-dialog .ai-model-head{display:flex;align-items:baseline;justify-content:space-between;gap:.5rem;}
.ai-dialog .ai-update{flex:none;padding:.15em .7em;border:1px solid var(--border, #8884);
  border-radius:999px;background:transparent;color:inherit;font:inherit;font-size:.75em;
  cursor:pointer;margin-bottom:.3em;}
.ai-dialog .ai-update:hover:not(:disabled){border-color:var(--accent, currentColor);}
.ai-dialog .ai-update:disabled{opacity:.5;cursor:default;}
.ai-dialog .ai-switch{margin-left:.4em;padding:0;border:0;background:none;color:inherit;
...
const AI_STRINGS = {
  pl: {
    ...
    discovered: "wykryty",
    tierBest: "Najlepszy", tierMid: "Zrównoważony", tierCheap: "Oszczędny",
    allModels: "Wszystkie modele",
    update: "Odśwież listę", updating: "Pobieram listę modeli…",
    updateHint: "Pobiera z API dostawcy najnowszy model w każdej kategorii.",
    updateNoKey: "Podaj klucz API, żeby pobrać listę modeli.",
    updated: n => `Zaktualizowano ${n} z 3 kategorii.`,
    updatedAt: d => `Ostatnia aktualizacja: ${d}.`,
    updateNoMatch: n => `Dostawca zwrócił ${n} modeli, ale żaden nie pasuje do znanych nazw kategorii.`,
    updateEmpty: "Dostawca nie zwrócił żadnych modeli.",
    updateFailed: code => `Nie udało się pobrać listy modeli (${code}).`,
    switchTo: id => `Nowszy model w tej kategorii: ${id}.`,
    switchAction: "przełącz",
  },
  en: {
    ...
    tierBest: "Best", tierMid: "Balanced", tierCheap: "Cost-effective",
    allModels: "All models",
    update: "Update list", updating: "Fetching the model list…",
    updateHint: "Pulls the newest model in each tier from the provider's API.",
    updateNoKey: "Add an API key to fetch the model list.",
    updated: n => `Updated ${n} of 3 tiers.`,
    updatedAt: d => `Last updated ${d}.`,
    updateNoMatch: n => `The provider returned ${n} models, none matching the known tier names.`,
    updateEmpty: "The provider returned no models.",
    updateFailed: code => `Could not fetch the model list (${code}).`,
    switchTo: id => `Newer model in this tier: ${id}.`,
    switchAction: "switch",
  },
};
...
function mountAiSelector({ chip, getLang, images = false }) {
  ...
  function renderDialog() {
    ...
    const known = effectiveModels(settings.provider, settings);
    const isCurated = known.includes(settings.model);
    dialog.innerHTML = `
      <h2>${t.title}</h2>
      <div class="ai-field">
      ...
      <div class="ai-field">
        <div class="ai-model-head">
          ...
          <button type="button" class="ai-update">${t.update}</button>
        </div>
        <select id="aiModelSelect"></select>
        <p class="ai-note ai-update-note" role="status" aria-live="polite"></p>
      </div>
    ...
    for (const [id, p] of Object.entries(PROVIDER_INFO)) {
      ...
      b.addEventListener("click", () => {
        ...
        settings.model = defaultModelFor(id, settings);
        save();
        renderDialog();
      });
    ...
    const tierLabels = { best: t.tierBest, mid: t.tierMid, cheap: t.tierCheap };
    // Populate the select: one group per capability tier, then everything
    // else under "all models". Models the curated catalog does not know are
    // tagged so the user can tell a discovered ID from a shipped one.
    let shownTiers = tierPicks(settings.provider, settings);
    const fillModels = (list, tiers) => {
      ...
      const addOption = (parent, id) => {
        ...
        opt.value = id;
        opt.textContent = info.models.includes(id) ? id : `${id} (${t.discovered})`;
        parent.appendChild(opt);
      };
      const promoted = new Set();
      for (const tier of TIER_IDS) {
        const id = tiers?.[tier];
        // A provider can name the same model in two tiers only if its
        // patterns overlap; listing it once keeps the select honest.
        if (!id || promoted.has(id)) continue;
        promoted.add(id);
      ...
      const rest = list.filter(id => !promoted.has(id));
      if (rest.length) {
        const group = document.createElement("optgroup");
        group.label = t.allModels;
        for (const id of rest) addOption(group, id);
        modelSel.appendChild(group);
      }
      modelSel.appendChild(customOpt());
      const selectable = [...promoted, ...rest];
      modelSel.value = selectable.includes(current) && current !== "__custom__" ? current
        : (selectable.includes(settings.model) ? settings.model : "__custom__");
    };
    fillModels(known, shownTiers);
    ...
    // ── "update list": refresh the three tiers from the provider's API ──
    // Explicit, per-provider (only that provider's key is on hand), and never
    // silent: unlike ambient discovery this reports why nothing happened.
    const updateBtn = dialog.querySelector(".ai-update");
    const updateNote = dialog.querySelector(".ai-update-note");
    const hasKey = () => Boolean(settings.keys[settings.provider]?.trim());
    const syncUpdateBtn = () => { updateBtn.disabled = !hasKey(); };
    const renderUpdateIdle = () => {
      const stamp = settings.catalog?.[settings.provider]?.updatedAt;
      updateNote.textContent = stamp
        ? t.updatedAt(new Date(stamp).toLocaleDateString(getLang() === "en" ? "en-GB" : "pl-PL"))
        : t.updateHint;
    };
    renderUpdateIdle();
    syncUpdateBtn();
    ...
    updateBtn.addEventListener("click", async () => {
      if (!hasKey()) { updateNote.textContent = t.updateNoKey; return; }
      const forProvider = settings.provider;
      const key = settings.keys[forProvider].trim();
      updateBtn.disabled = true;
      updateBtn.setAttribute("aria-busy", "true");
      updateNote.textContent = t.updating;
      let result;
      try {
        result = await updateProviderModels(forProvider, key);
      } catch {
        result = { tiers: {}, ids: [], total: 0, matched: [], error: "network" };
      }
      updateBtn.removeAttribute("aria-busy");
      // The user may have switched providers or closed the dialog mid-fetch;
      // renderDialog has replaced these nodes, so the result is stale.
      if (!dialog.open || settings.provider !== forProvider) return;
      syncUpdateBtn();
      if (!result.matched.length) {
        updateNote.textContent = result.error === "empty" ? t.updateEmpty
          : result.error === "no-match" ? t.updateNoMatch(result.total)
          : t.updateFailed(result.error);
        return;
      }
      // Store only the tiers that matched: an unmatched tier keeps whatever
      // the curated catalogue says rather than being blanked.
      const tiers = {};
      for (const tier of TIER_IDS) if (result.tiers[tier]) tiers[tier] = result.tiers[tier];
      settings.catalog = { ...settings.catalog, [forProvider]: { tiers, updatedAt: Date.now() } };
      save();
      fillModels([...new Set([...effectiveModels(forProvider, settings), ...result.ids])],
        tierPicks(forProvider, settings));
      // A partial walk still updates what it found, and still says so.
      updateNote.textContent = result.error
        ? `${t.updated(result.matched.length)} ${t.updateFailed(result.error)}`
        : t.updated(result.matched.length);
      // Never move the user off a model they picked; offer the newer one.
      const suggestion = tiers[classifyModel(forProvider, settings.model)];
      if (suggestion && suggestion !== settings.model) {
        updateNote.append(` ${t.switchTo(suggestion)} `);
        const switchBtn = document.createElement("button");
        switchBtn.type = "button";
        switchBtn.className = "ai-switch";
        switchBtn.textContent = t.switchAction;
        switchBtn.addEventListener("click", () => {
          settings.model = suggestion;
          save();
          modelSel.value = suggestion;
          renderUpdateIdle();
        });
        updateNote.appendChild(switchBtn);
      }
    });
    ...
    if (discoverKey && !settings.provider.startsWith("__")) {
      ...
      discoverProviderModels(forProvider, discoverKey).then(list => {
        ...
        if (extra.length) fillModels([...new Set([...known, ...list])], shownTiers);
